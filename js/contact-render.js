...
const PHONE_ICON_SMALL = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>`;

const EMAIL_ICON_SMALL = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>`;
...
function renderBranch(branch) {
  const headerHTML = branch.header
    ? `<div class="section-header"><h2>${branch.header.title}</h2><p>${branch.header.subtitle}</p></div>`
    : "";

  const cardsHTML = branch.cards.map((card) => renderCard(card)).join("");

  const mapHTML = branch.map
...
function renderCard(card) {
  const iconHTML = CONTACT_ICONS[card.icon] || "";

  let bodyHTML = "";

  switch (card.type) {
    case "phone":
      bodyHTML = `<div class="phone-btn-group">${card.items.map((p) => `<a href="tel:${p.number}" class="phone-btn">${PHONE_ICON_SMALL}${p.display} <span class="phone-role">${p.role}</span></a>`).join("")}</div>`;
      break;
    case "email":
      bodyHTML = `<div class="email-link-group">${card.items.map((e) => `<a href="mailto:${e.address}" class="email-btn">${EMAIL_ICON_SMALL}${e.address} <span class="phone-role">${e.role}</span></a>`).join("")}</div>`;
      break;
    case "address":
    case "hotline":
    case "email-simple":
