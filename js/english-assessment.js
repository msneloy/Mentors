...
const QS = [
  ...
  },
  {
    ...
    opts: { A: "were", B: "had been", C: "would be", D: "would have been" },
    ans: "B",
  ...
  },
  {
    ...
    opts: { A: "which", B: "that", C: "whose", D: "who" },
    ans: "C",
  ...
  },
  {
    ...
    opts: { A: "Although", B: "Despite", C: "Because", D: "Since" },
    ans: "B",
...
/* ── State ─────────────────────────────────────────────── */
let S = { idx: 0, ans: {}, timeLeft: 180000, timer: null, hlMode: false };
function initTheme() {
  const saved = localStorage.getItem("ea-theme") || "dark";
...
function toggleTheme() {
  ...
  if (btn) btn.innerHTML = next === "light" ? svgMoon() : svgSun();
}
...
function isCorrect(q, ua) {
  ...
  const n = (s) =>
    s
      ...
      .replace(/[\s\u2011\u2012\u2013-]/g, "");
  const c = n(ua);
  if (q.type === "mcq") return n(q.ans) === c;
  if (n(q.ans) === c) return true;
  return (q.alt || []).some((a) => n(a) === c);
}
...
function renderExam() {
  ...
  <div class="exam">
    <div class="topbar">
      ...
      <div class="topbar-right">
        ...
        <button class="theme-btn" id="theme-toggle-btn" onclick="toggleTheme()" title="Toggle theme">${isDark ? svgSun() : svgMoon()}</button>
  ...
  if (isReading) setupHighlight();
}

/* ── Q-nav chips ───────────────────────────────────────── */
function buildQNav() {
  let h = '<span class="qnav-label">Reading</span>';
  QS.forEach((q, i) => {
    if (i === 10)
      h +=
        '<div class="qnav-sep"></div><span class="qnav-label">Grammar</span>';
    const cls = i === S.idx ? "cur" : S.ans[q.id] !== undefined ? "done" : "";
...
/* ── Question body ─────────────────────────────────────── */
function buildQBody(q) {
  const label = q.sec === 1 ? "Section 1 — Reading" : "Section 2 — Grammar";
  let body = `<div class="sec-badge">${label}</div><div class="q-text">${q.id}. ${q.q}</div>`;
  if (q.type === "mcq") {
    body += '<div class="opts">';
    for (const [l, t] of Object.entries(q.opts)) {
      const sel = S.ans[q.id] === l ? "sel" : "";
      body += `<div class="opt ${sel}" data-l="${l}"><div class="opt-ltr">${l}</div><div class="opt-txt">${t}</div></div>`;
    }
    body += "</div>";
  } else {
    const val = esc(S.ans[q.id] || "");
...
function buildQFooter() {
  ...
  return `<div class="q-footer">
    ...
    ${
      isLast
        ? `<button class="btn-nav-p" id="btn-next" onclick="showConfirm()">Submit ${svgRight()}</button>`
        : `<button class="btn-nav-p" id="btn-next">Next ${svgRight()}</button>`
    }
  </div>`;
}
...
function saveInput() {
  const inp = $("txt-inp");
  if (inp) S.ans[QS[S.idx].id] = inp.value.trim() || undefined;
}

function updateQNav() {
  const nav = $("qnav");
  if (nav) ((nav.innerHTML = buildQNav()), bindNavChips());
}
...
function setupHighlight() {
  ...
  btn.onclick = () => {
    ...
    document.getElementById("passage-content").style.cursor = S.hlMode
      ? "text"
      : "default";
  };
...
/* ── Timer (centiseconds, updates every 10ms) ─────────── */
function startTimer() {
  ...
  S.timer = setInterval(() => {
    ...
    if (el) {
      ...
      const totalSec = Math.floor(S.timeLeft / 100);
      el.className =
        "timer" + (totalSec < 300 ? " danger" : totalSec < 600 ? " warn" : "");
    }
    if (S.timeLeft <= 0) {
...
function showConfirm() {
  saveInput();
  const answered = QS.filter((q) => S.ans[q.id] !== undefined).length;
...
function submitExam(timeout) {
  ...
  let score = 0;
  QS.forEach((q) => {
    if (isCorrect(q, S.ans[q.id])) score++;
  });
  renderResults(score, timeout);
}

/* ── Results ───────────────────────────────────────────── */
function renderResults(score, timeout) {
  const ielts = score >= 15;
  const cId = ielts ? "ielts-advanced" : "ielts";
  const cName = ielts ? "IELTS — Academic & General" : "Basic to IELTS";
  const cDesc = ielts
    ? "Your score shows a solid English foundation. We recommend jumping straight into intensive IELTS preparation to focus on exam strategy and mock tests."
    : "A high IELTS band requires strong grammar and reading skills. We recommend starting with our Basic to IELTS programme to build your core language skills first.";
  const dashOffset = 476 - (476 * score) / 20;
  const isDark =
    (document.documentElement.getAttribute("data-theme") || "dark") !== "light";
  ...
  <div class="exam" style="overflow:hidden">
    <div class="topbar">
      ...
      <div class="topbar-right">
        <button class="theme-btn" id="theme-toggle-btn" onclick="toggleTheme()" title="Toggle theme">${isDark ? svgSun() : svgMoon()}</button>
        <button class="btn-r" onclick="renderIntro()">Retake Test</button>
      </div>
    </div>
  ...
  $("review-toggle-btn").onclick = function () {
    const rc = $("review-container");
    const hidden = rc.style.display === "none";
    rc.style.display = hidden ? "block" : "none";
    this.textContent = hidden ? "Hide Review" : "Review Answers";
    if (hidden) setTimeout(() => rc.scrollIntoView({ behavior: "smooth" }), 50);
  };
}
...
function buildReview() {
  return QS.map((q) => {
    ...
    let display_ua = ua || "(no answer)";
    let display_ans = q.ans;
    if (q.type === "mcq") {
      display_ua = ua ? `${ua}. ${q.opts[ua] || ""}` : "(no answer)";
      display_ans = `${q.ans}. ${q.opts[q.ans]}`;
    }
    const secLabel = q.sec === 1 ? "Reading" : "Grammar";
    return `
    <div class="rq">
      <div class="rq-head">
