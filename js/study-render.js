...
(function() {
  /*
    Study Abroad Logic
  */
  var renderStudyDetail, renderStudyOverview;

  renderStudyDetail = function() {
    var country, countryId, root, uniCards;
    root = document.getElementById('study-detail-root');
    if (!root) {
      return;
    }
    countryId = location.hash.replace("#", "") || location.search.replace("?country=", "");
    country = window.STUDY_COUNTRIES.find(function(c) {
      return c.id === countryId;
    });
    if (!country) {
      root.innerHTML = `<div style="text-align:center; padding:100px 20px;">
  ...
  <a href="study-abroad.html" class="btn btn-primary">Back to Overview</a>
    ...
    uniCards = country.universities.map(function(uni) {
      var locationHtml;
      locationHtml = uni.location ? `<span class=\"uni-location\">${uni.location}</span>` : "";
  ...
  ${locationHtml}
    ...
    return root.innerHTML = `<div class="page-hero">
  <div class="container">
...
<style>
  ...
  renderStudyOverview = function() {
    var benefitsHtml, cards, certsHtml, contentHtml, contentRoot, data, destinationsRoot, heroContainer, pointsHtml, servicesHtml;
  ...
  <a href="study-detail.html#${c.id}" style="color:var(--accent); font-size:0.9rem; margin-top:15px; display:inline-block; text-decoration:none; font-weight: 500;">View Details →</a>
      ...
      destinationsRoot.innerHTML = `<div style="text-align: center; margin-bottom: 50px">
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 30px; margin-bottom: 60px;">
  ${cards}
    ...
    if (contentRoot) {
      ...
      // Why & Benefits
      if (window.STUDY_PAGE_CONTENT.whySection && window.STUDY_PAGE_CONTENT.benefits) {
        pointsHtml = window.STUDY_PAGE_CONTENT.whySection.points.map(function(p) {
          return `<li style="margin-bottom: 15px; padding-left: 30px; position: relative; color: var(--text-main);">
  <span style="position: absolute; left: 0; color: var(--accent)">✓</span>
  <strong>${p.title}</strong> ${p.desc}
</li>`;
        }).join('');
        certsHtml = window.STUDY_PAGE_CONTENT.whySection.certifications.map(function(c) {
          return `<span style="background: var(--surface-2); padding: 8px 15px; border-radius: 6px; font-size: 0.9rem; color: var(--text-main);">${c}</span>`;
        }).join('');
        benefitsHtml = window.STUDY_PAGE_CONTENT.benefits.items.map(function(i) {
          return `<div style="background: var(--bg-light); border: 1px solid var(--border-color); padding: 20px; border-radius: 8px; font-size: 1.2rem; font-weight: bold; color: var(--text-main);">${i}</div>`;
        }).join('');
        contentHtml += `<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px; margin-bottom: 60px;">
  ...
  <div style="background: var(--surface); border-radius: 12px; padding: 40px; box-shadow: var(--shadow-sm); border: 1px solid var(--border-color); display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center;">
    ...
    <p style="margin-top: 20px; font-size: 0.8rem; color: var(--text-muted);">${window.STUDY_PAGE_CONTENT.benefits.footnote}</p>
      ...
      if (window.STUDY_PAGE_CONTENT.services) {
        servicesHtml = window.STUDY_PAGE_CONTENT.services.items.map(function(s, idx) {
          return `<div style="display: flex; align-items: center; background: var(--surface-2); padding: 15px 25px; border-radius: 50px; border: 1px solid var(--border-color);">
  ...
  <h4 style="font-size: 1.05rem; margin: 0; color: var(--text-main)">${s}</h4>
        ...
        contentHtml += `<div style="background: var(--surface); border-radius: 12px; padding: 50px 40px; box-shadow: var(--shadow-sm); border: 1px solid var(--border-color); margin-bottom: 60px;">
  ...
  <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px;">
    ${servicesHtml}
      ...
      return contentRoot.innerHTML = contentHtml;
    }
  };
